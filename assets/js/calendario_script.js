(() => {
    ...
    function renderTable(lista) {
        ...
        refs.tableBody.innerHTML = lista.map(c => `
            <tr>
                ...
                <td>${fmtData(c.alterado_em || c.data_criacao)}</td>
                <td>
    ...
    function gerarResumoFinalCal() {
        ...
        const isFeriadosOn = document.querySelector('input[name="considerarFeriados"]:checked')?.value === 'sim';
        const txtFeriadosAPI = isFeriadosOn ? 'Contabilizados como letivos' : 'Contabilizados como recessos (Automático)';
        ...
        resumoDiv.innerHTML = `
            ...
            <ul class="list-disc pl-5 space-y-1">
                ...
                <li><strong>Feriados Nacionais/Estaduais/Municipais:</strong> ${txtFeriadosAPI}</li>
    ...
    function setupEvents() {
        ...
        refs.tableBody?.addEventListener('click', async (e) => {
            ...
            if (btn.classList.contains('btn-delete')) {
                alert("Atenção: O calendário não pode ser excluído definitivamente. Para removê-lo das listagens, altere seu status para 'Inativo' na edição.");
                return;
            }
            ...
            if (btn.classList.contains('btn-view')) {
                ...
                if (cal) {
                    ...
                    document.getElementById('auditInfoUser').style.display = 'block';
                    const fDataHora = (d) => d ? App.format.fmtDateTimeBR(d) : '--/--/----';
                    document.getElementById('viewDataCriacao').textContent = fDataHora(cal.data_criacao);
                    document.getElementById('viewDataAtualizacao').textContent = fDataHora(cal.alterado_em || cal.data_criacao);
                    document.getElementById('viewAlteradoPor').textContent = cal.alterado_por || 'Sistema';
    ...
    function initFullCalendar(cal, isInteractive = false) {
        ...
        calendarInstance = new FullCalendar.Calendar(el, {
            ...
            dateClick: function(info) {
                if (isInteractive) openAcoesDiaModal(info.dateStr, false);
            },
            eventClick: function(info) {
                if (isInteractive) openAcoesDiaModal(info.event.startStr, true, info.event);
            },
    ...
    function openAcoesDiaModal(dateStr, hasEvent, eventObj = null) {
        const modal = document.getElementById('acoesDiaModal');
        document.getElementById('acoesDiaData').innerText = dateStr.split('-').reverse().join('/');
        
        const btnAdd = document.getElementById('btnAcaoAdicionar');
        const btnEdit = document.getElementById('btnAcaoEditar');
        const btnRemove = document.getElementById('btnAcaoRemover');
        
        if (hasEvent) {
            btnAdd.disabled = true; btnAdd.classList.add('opacity-50');
            btnEdit.disabled = false; btnEdit.classList.remove('opacity-50');
            btnRemove.disabled = false; btnRemove.classList.remove('opacity-50');
            
            btnEdit.onclick = () => { App.ui.hideModal(modal); openDiaModalEdit(eventObj); };
            btnRemove.onclick = () => { 
                App.ui.hideModal(modal); 
                $('#diaId').value = eventObj.id;
                document.getElementById('btnDeleteDia').click(); 
            };
        } else {
            btnAdd.disabled = false; btnAdd.classList.remove('opacity-50');
            btnEdit.disabled = true; btnEdit.classList.add('opacity-50');
            btnRemove.disabled = true; btnRemove.classList.add('opacity-50');
            
            btnAdd.onclick = () => { 
                App.ui.hideModal(modal); 
                openDiaModal(); 
                document.getElementById('inicioDia').value = dateStr; 
                document.getElementById('inicioDia').disabled = true;
            };
        }
