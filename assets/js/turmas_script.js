    ...
    Object.entries(curso.unidade_curricular).forEach(([ucId, dadosUc], index) => {
        ...
        div.innerHTML = `
            ...
            <div class="mt-3">
                <label class="inline-flex items-center text-sm text-blue-600 cursor-pointer">
                    <input type="checkbox" class="form-checkbox h-4 w-4 check-subs" ${saved && saved.possui_substituto ? 'checked' : ''}>
                    <span class="ml-2">Possui Substituto Temporário?</span>
                </label>
            </div>

            <div class="substituto-box mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded hidden">
                 <div class="grid grid-cols-12 gap-3">
                    <div class="col-span-12 md:col-span-6">
        ...
        chk.addEventListener('change', () => {
            if(chk.checked) {
                ...
                // Limpa campos ao esconder? Opcional.
            }
        });
